...
std::array<GLfloat, 3 * (15 + 2)> vertices;////tu i niżej
std::array<int, (15 + 1) * 2 * 3> triangles;
...
void BuildPyramid()
{
	vertices[3 *  15 + 0] = 0.0f;
	vertices[3 *  15 + 1] = 0.0f;
	vertices[3 *  15 + 2] = 0.0f;
	vertices[3 * (15 + 1) + 0] = 0.0f;
	vertices[3 * (15 + 1) + 1] = 0.0f;
	vertices[3 * (15 + 1) + 2] = 1.0f;

	// podstawa
	for (int i = 0; i < 15; ++i)
	{
		const GLfloat x = 0.5f * std::sin(2.0f * 3.1415f * ((GLfloat)i / 15));
		const GLfloat y = 0.5f * std::cos(2.0f * 3.1415f * ((GLfloat)i / 15));
	...
	{
		triangles[3 * i + 0] = 15;
		triangles[3 * i + 1] = i % 15;
		triangles[3 * i + 2] = (i + 1) % 15;

		triangles[3 * (i + 15 + 1) + 0] = (i + 1) % 15;
		triangles[3 * (i + 15 + 1) + 1] =		i % 15;
		triangles[3 * (i + 15 + 1) + 2] =			15 + 1;
	}
}
